...
const percentageOfHabitablePlanets = computed<number>(() => {
  ...
  const habitablePlanets = planets.filter(
    (planet) => planet.oxygenPercentage >= 19 && planet.oxygenPercentage <= 24
  );
...
<template>
  ...
  <h2>Planets</h2>
  <p>
    <i v-if="planetCount === 0"> We have discovered no planets yet. </i>
    <i v-else> We have discovered {{ planetCount }} planets so far. </i>
  </p>
  <p>
    ...
    <i v-else>
      Hooray! 🥳
      {{ percentageOfHabitablePlanets.toFixed(1) }}% of these planets are
      habitable. Let's hop into the rocket! 🚀
    </i>
  </p>
  <ul>
    <li v-for="planet in planets" :key="planet.name">
      <strong>{{ planet.name }}</strong> with {{ planet.oxygenPercentage }}%
      oxygen
      <a href="#" @click="removePlanet(planet)">Remove</a>
    </li>
  </ul>

  <button v-if="!state.showForm" @click="state.showForm = true">
    Add planet
  </button>
  <div v-else>
    <h4>Add planet</h4>
